...
from src.log import save_model, save_config, save_result
...
def logging(train_val_accuracy, model, starttime, batch_size, nb_epoch, conv_arch,dense, dropout,
            X_shape, y_shape, train_acc, val_acc, dirpath):
    now = time.ctime()
    #f = '../data/weights/{}'.format(now)
    f = '../data/weights'
    model.save_weights(f)
    #save_model(model.to_json(), now, dirpath)
    save_model(model.to_json(),dirpath)
    #save_config(model.get_config(), now, dirpath)
    save_config(model.get_config(), dirpath)
    # save_result(starttime, batch_size, nb_epoch, conv_arch, dense, dropout,
    #                 X_shape, y_shape, train_acc, val_acc, dirpath)
    #save_result(train_val_accuracy=train_val_accuracy, notes = "",conv_arch=conv_arch, dense=dense)
...
def cnn_architecture(X_train, y_train, conv_arch=[(32,3),(64,3),(128,3)],
    ...
    # model architecture:
    model = Sequential()
    model.add(Conv2D(filters=conv_arch[0][0], kernel_size=3,strides= 3, padding='same', activation='relu',input_shape=(1, X_train.shape[2], X_train.shape[3])))


    if (conv_arch[0][1]-1) != 0:
        for i in range(conv_arch[0][1]-1):
            model.add(Conv2D(conv_arch[0][0],kernel_size=3, strides=3, padding='same', activation='relu'))
        model.add(MaxPooling2D(pool_size=(2, 2),padding='same'))

    if conv_arch[1][1] != 0:
        for i in range(conv_arch[1][1]):
            model.add(Conv2D(conv_arch[1][0], kernel_size=3, strides=3, padding='same', activation='relu'))
        model.add(MaxPooling2D(pool_size=(2, 2),padding='same'))

    if conv_arch[2][1] != 0:
        for i in range(conv_arch[2][1]):
            model.add(Conv2D(conv_arch[2][0],kernel_size=3, strides=3 ,padding='same', activation='relu'))
        model.add(MaxPooling2D(pool_size=(2, 2),padding='same'))
    ...
    logging(train_val_accuracy, model, starttime, batch_size, nb_epoch, conv_arch, dense,
